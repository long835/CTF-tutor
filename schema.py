...
class ArchiveEntry:
    ...
    def from_dict(cls, data: dict) -> "ArchiveEntry":
        """
        Build an entry from a raw dict, tolerating older field names.

        Early pattern cards used `approach` and `tools` where the current
        schema uses `explanation` and `tools_used`. Being strict here meant
        those files silently failed to load and never got indexed, so the
        aliases are mapped and unknown keys are ignored instead.
        """
        ...
        for old, new in aliases.items():
            if old in payload and not payload.get(new):
        ...
        if unknown:
            # Preserve, rather than discard, anything we did not recognise.
            extra = "; ".join(f"{k}={payload[k]!r}" for k in unknown)
            cleaned["notes"] = ((cleaned.get("notes") or "") + f" [unmapped: {extra}]").strip()

        missing = [f for f in ("challenge_name", "category", "techniques") if not cleaned.get(f)]
        if missing:
            raise ValueError(f"archive entry missing required field(s): {', '.join(missing)}")
